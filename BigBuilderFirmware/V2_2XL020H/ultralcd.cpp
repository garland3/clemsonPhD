...
//static void lcd_control_extruder_menu();
static void lcd_control_extruder_ratio_menu();
...
static void lcd_status_screen()
{
    ...
    if (lcdDrawUpdate)
    {
        ...
        lcd_status_update_delay = 10;   /* redraw the main screen every second. This is easier then trying keep track of all things that change on the screen *aangepast op 18-10 */
    }
#ifdef ULTIPANEL
    if (LCD_CLICKED)
    {
...
void lcd_preheat_pla()
{
    ...
    TargetHotend = plaPreheatHotendTemp;
    preheat=true;
    PreheatFanSpeed = plaPreheatFanSpeed;
...
void lcd_preheat_abs()
{
    ...
    TargetHotend = absPreheatHotendTemp;
    preheat=true;
    PreheatFanSpeed = absPreheatFanSpeed;
    lcd_return_to_status();
    setWatch(); // heater sanity check timer
}
...
static void lcd_move_z()
{
    if (encoderPosition != 0)
    {
        ...
        #else
        plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], manual_feedrate[Z_AXIS]/30, active_extruder);
        #endif
        lcdDrawUpdate = 1;
    }
    if (lcdDrawUpdate)
...
{  
       static float tmp_ratio;
       static int tmp_extruder;
       tmp_ratio= ratio;
       ratio=100;
    ...
    if (lcdDrawUpdate)
    {
        lcd_implementation_drawedit(PSTR("Extr. Rear"), ftostr31(current_position[E_AXIS]));
    }
    if (LCD_CLICKED)
    {
        lcd_quick_feedback();
...
{      
    ...
    if (lcdDrawUpdate)
    {
        lcd_implementation_drawedit(PSTR("Extr. Front"), ftostr31(current_position[E_AXIS]));
    }
    if (LCD_CLICKED)
    {
        lcd_quick_feedback();
...
static void lcd_move_menu_axis()
{
    ...
    if (move_menu_scale < 10.0)
    {
        ...
        MENU_ITEM(submenu, "Extr. Rear", lcd_move_e);
        MENU_ITEM(submenu, "Extr. Front", lcd_move_e2);
    }
    END_MENU();
}
...
static void lcd_control_menu()
{
    START_MENU();
    MENU_ITEM(back, MSG_MAIN, lcd_main_menu);
 //   MENU_ITEM(submenu, MSG_EXTRUDERS, lcd_control_extruder_menu); // *aangepast
    MENU_ITEM(submenu, MSG_EXTRUDER_RATIO, lcd_control_extruder_ratio_menu);
...
//{
 //   START_MENU();
 //   MENU_ITEM(back, MSG_CONTROL, lcd_control_menu);
 //   MENU_ITEM(function, MSG_TOGGLE, toggle_extruder);
 //   END_MENU();
//} 
...
static void lcd_control_extruder_ratio_menu()
{
   static float tmp_ratio;
    START_MENU();
    MENU_ITEM(back, MSG_CONTROL, lcd_control_menu);
    MENU_ITEM_EDIT(float3, MSG_EXTRUDER_RATIO_1, &ratio, 0, 100);
    lcd.setCursor(19, 1);
    lcd_printPGM(PSTR("%"));
    tmp_ratio=(100-ratio);
    lcd.setCursor(10, 2);
    lcd_printPGM(PSTR("Front:"));
    lcd.print(itostr3(tmp_ratio));
    lcd.setCursor(19, 2);
    lcd_printPGM(PSTR("%"));
    END_MENU();
}
