...
isRightEdge(nn) = 1;
rightNodes = [0;0];
%isRightEdge will have true for all of the nodes on the right edge.
...
for i = 1:nn
    ...
    if(xLoc == 10)
         ...
         if(yLoc==0||yLoc==1) 
             isRightEdge(2*i)=1/2;
         end;%The top and bottom nodes get half of the force of the other 
         %nodes.
         rightNodes = [rightNodes,[xLoc;yLoc]];
    end
end
...
for i=1:ne
    ...
    vonM_BTdiff(i) = (vonM_stored(i)-vonM_BT(i))/vonM_BT(i);
    UyBTdiff(i) =(U(2*i)-UyBT(i))/UyBT(i);
end
