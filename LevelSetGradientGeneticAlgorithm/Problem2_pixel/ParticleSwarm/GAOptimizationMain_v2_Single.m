function GAOptimizationMain_v2_Single(args)
...
nvar = 21;
LB = ones(1,nvar)*0;   % Lower bound = -2
UB = ones(1,nvar)*1;   % Upper bound = 2
...
% Testing options
if(runningLocal ==1)
    gaoptions = optimoptions('particleswarm','Display','iter','SwarmSize',5,  ...
        'MaxIter',200,'StallIterLimit',10);
else
    % actual values, I want to use. 
    % population size = 5*23 where 23 is number of worker threads
    gaoptions = optimoptions('particleswarm','Display','iter','SwarmSize',nvar*2, ...
        'MaxIter',200,'StallIterLimit',30);
end
...
if(runParallel == 1)   
    ...
    n = matlabpool('size') 
    gaoptions = optimoptions(gaoptions,'UseParallel','always');
...
constF = conF;
counter = 1;
[x,fval, exitflag, output] = particleswarm(fun,nvar,LB,UB,gaoptions);
