...
if(problem ==1)
    ...
    if(doplot ==1)
        ...
        sizeTemp = size(inchTicks);
        mmTicks = 0:50:50*sizeTemp(2);
        set(ax,'XTickLabel',mmTicks );
        
       % xlabel('X distance along beam in mm','FontSize',12, 'FontName','Arial')
          xlabel('X distance along beam in mm')
    end
...
inchTicksY = -0.393701*2:0.393701*2:0.393701*3*2;
set(ax,'YTick',inchTicksY ); % 50 mm = 1.9685 inches
%sizeTemp = size(inchTicksY);
mmTicks = -10*2:10*2:10*3*2;
