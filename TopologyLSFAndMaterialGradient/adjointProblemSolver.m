function [ p ] = adjointProblemSolver(  nelx,nely, load)

nn = 25; % number of nodes
ne = 16; % number of elements
% row is the particular elmenent, the columns represent local nodes The
% values at each position represent the global node number We are numbering
% elements counter clockwise. 1 is in the bottom left corner of each
% element as element 2
if(1==1)
    ...
    for j= 0:3
        step=j*4;
        step2=j*5;
        IEN(1+step,:) = [1+step2 2+step2 7+step2 6+step2];
        IEN(2+step,:) = [2+step2 3+step2 8+step2 7+step2];
        IEN(3+step,:) = [3+step2 4+step2 9+step2 8+step2];
        IEN(4+step,:) = [4+step2 5+step2 10+step2 9+step2];
