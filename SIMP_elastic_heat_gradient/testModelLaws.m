...
% redo calculation for linear case. 
settings.elasticMaterialInterpMethod = 1; % linear
settings.heatMaterialInterpMethod = 1; % linear
count = 1;
for i = volfractionsM1
    eff2(count) =  mat.effectiveElasticProperties( i, settings);
    effH2(count) = mat.effectiveHeatProperties(i,settings);
    count = count+1;
end
...
hold on
plot(volfractionsM1,eff2)
hold off

subplot(2,1,2)
plot(volfractionsM1,effH)
hold on
plot(volfractionsM1,effH2)
hold off
