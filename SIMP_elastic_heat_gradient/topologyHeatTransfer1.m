function topologyHeatTransfer1()

ActualtopHeat(60,60,0.3,3,2)
%%%% A 99 LINE TOPOLOGY OPTIMIZATION CODE BY OLE SIGMUND, JANUARY 2000 %%%
%%%% CODE MODIFIED FOR INCREASED SPEED, September 2002, BY OLE SIGMUND %%%
function [x] =  ActualtopHeat(nelx,nely,volfrac,penal,rmin)
...
while change > 0.01  
  ...
  xold = x;
% FE-ANALYSIS
  [U]=temperatureFEA2(nelx,nely,x,penal);   
...
for ely = 1:nely
    rowMultiplier = ely-1;
    for elx = 1:nelx
           columnNodes=[rowMultiplier*elementsInRow+elx;
                    rowMultiplier*elementsInRow+elx+1;
                    (rowMultiplier +1)*elementsInRow+elx+1;
                    (rowMultiplier +1)*elementsInRow+elx];
         Ue = U(columnNodes,:);
         
         xlocal = x(ely,elx)^penal;
         Klocal = kmaterial1*xlocal+(1-xlocal)*kmaterial2;
