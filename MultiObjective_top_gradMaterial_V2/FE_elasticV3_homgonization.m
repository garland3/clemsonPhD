function [designVars]=FE_elasticV3_homgonization(designVars, settings, matProp,macroElemProps)
...
for loadcaseIndex = 1:t2
    for e = 1:ne
        ...
        strainelementcase = macroElemProps.strain(t2,:)'; 
        if (1==2)
      
            innerTempSens = transpose(temp3)*designVars.x(y,x)^(settings.penal-1)*settings.penal*temp3;
            sensitivity = transpose(strainelementcase)*(innerTempSens)*strainelementcase;
        else
            strainelementcaseArray = [strainelementcase strainelementcase       strainelementcase];
            baseDSensitivity = designVars.x(y,x)^(settings.penal-1)*settings.penal*D;
            strainMacroMinusMeso = (strainelementcaseArray-temp2_BX);
            sensitivity =baseDSensitivity*strainMacroMinusMeso*strainMacroMinusMeso;
            sensitivity = sum(sum(sensitivity));
        end
        
        
        designVars.temp1(y,x) =designVars.temp1(y,x)+ sensitivity;
