function [U,angleStored] = FEA_elastic_V2(nelx,nely,x,penal,rotM,E1,E2,v1,v2,G)
% FEA
...
Essential2   =union( 1:2*numNodesInRow :2*(numNodesInColumn-1)*numNodesInRow+1,numNodesInRow*2) ;% X direction on left side is held fixed
               % Y direction on left side is held fixed
Essential2 = unique(Essential2);
alldofs     = [1:ndof];
Free    = setdiff(alldofs,Essential2);

F2(2*(numNodesInColumn-1)*numNodesInRow+2,1) = -1000; % y direction, down
Essential2 = unique(Essential2);
...
for e = 1:ne         
     ...
     % calculating the objective function
      ...
      K(dofNumbers,dofNumbers) = K(dofNumbers,dofNumbers) + t*ke* x(yLoc,xLoc)^penal;
...
yLoc = 1;
 for e = 1:ne
    arrayCoordNumber = zeros(1,4);
     ...
     D  = transpose(TransFormMatrix)*Dtemp*TransFormMatrix;  
...
u(1) = 10;
v(1) = 10;
u = transpose(cos(  angleStored));
v = transpose(sin(  angleStored));
subplot(2,2,2)
quiver(x,y,u ,v);
xlim([0,nelx])
%ylim([0,h])
