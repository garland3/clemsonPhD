...
if(macroDesignMode==1)
    while  masterloop<=config.maxMasterLoops && FEACalls<=config.maxFEACalls
        ...
        % --------------------------------
%         if(config.useOrthDistribution ==1)
%             if ( config.mode==3 || config.mode ==55)
%                 % --------------------------------
%                 % Re - Run FEA, calculate sensitivities
%                 % --------------------------------
%                 DV = DV.RunFEAs(config, matProp, masterloop);
%                 FEACalls = FEACalls+1;
%                 opt.OptimizeOrthoDistribution(DV, config, matProp,masterloop)
%                 DV.storeOptimizationVar = [DV.storeOptimizationVar;DV.c, DV.cCompliance, DV.cHeat,DV.currentVol1Fraction,DV.currentVol2Fraction,DV.currentVol1Fraction/DV.currentVol2Fraction,sum(sum(DV.x))];
%                 ShowOptimizerProgress(DV,1,' orth D',FEACalls,config, matProp);  
...
[DVMeso, mesoconfig] = GenerateDesignVarsForMesoProblem(config,1);
...
for e = 1:ne
    ...
    % Check if void
    if(macroElementProps.density>config.voidMaterialDensityCutOff)
        ...
        % -------------------------------------
        if(postProcess~=1)
            yShift = (macroElementProps.yPosition-1)*mesoconfig.nely*numTilesY+1;
            xShift = (macroElementProps.xPosition-1)*mesoconfig.nelx*numTilesX+1;
