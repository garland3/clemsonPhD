...
if(config.UseLookUpTableForPsuedoStrain==1 && config.strainAndTargetTest~=1 && config.multiscaleMethodCompare~=1)
            ...
            for j= fliplr(1:t2)
                ...
                for k = 1:t3
                    ...
                    for l = 1:t4
                        ...
                        if(1==1)
                            ps(1)=pstrain1(i,j,k,l);
                            ps(2)=pstrain2(i,j,k,l);
                            ps(3)=pstrain3(i,j,k,l);
                        end
...
%                         diffValue = (D11_table-D11sys)^2+(D22_table-D22sys)^2+(D33_table-D33sys)^2;
                        ...
                        test1=diffValue<(minValue+minValue*errorOnMinAllowed) &&  etaLocal<bestEtaForMinValue ;
                        test2= diffValue<(minValue-minValue*errorOnMinAllowed);
                        if(test1 ||  test2) % allow up to errorOnMinAllowed higher values.
                            %                             if(|| )) % then compare the density
                            minValue=diffValue;
...
%             diffValue = (D11_table2-D11sys)^2+(D22_table2-D22sys)^2+(D33_table2-D33sys)^2;
    ...
    elseif(scaleVersion==3)
        D11_table2=D11_table;
        D12_table2=D12_table;
        D22_table2=D22_table;
        D33_table2=D33_table;
       bestScale=0;
       etaTargetLocal=originalEta;
        %                     % --------------------
        %                     % Version 2
        %                     % --------------------
        %
    ...
    fprintf('Targets\t\t\t\tD11 %f D12 %f  D22 %f D33 %f\n', D11sys,D12sys,D22sys,D33sys);
    i=indexOfMinValue;
    fprintf('Table Values\t\tD11 %f D12 %f  D22 %f D33 %f Eta %f\n', D11_table,D12_table,D22_table,D33_table,originalEta);
    fprintf('Scaled Table Values\tD11 %f D12 %f D22 %f D33 %f\nIndex is at %i,%i,%i,%i\nbestscale %f\nEta %f\n', D11_table2,D12_table2,D22_table2,D33_table2,i(1),i(2),i(3),i(4),bestScale,etaTargetLocal);
    fprintf('Beta %f %f %f \n', ps(1),ps(2),ps(3));
elseif(version== 3)
    %                 % ------------------------------------
    %                 %           Version 3
...
elseif(version== 4)
    ...
    %----------------------------------
    % Find out what the best fit values actually werer
    %----------------------------------
    ...
    [D11_interp,D12_interp ,D22_interp,D33_interp]= InterpolatePseudoStrainsAndDensity(ps,etaTargetLocal, ...
        D11_table4D,D12_table4D,D22_table4D,D33_table4D,...
        ps1_table4D,ps2_table4D,ps3_table4D,eta_table4D);
    
    fprintf('Targets\t\t\t\tD11 %f D12 %f  D22 %f D33 %f\n', D11sys,D12sys,D22sys,D33sys);
    fprintf('Best Values\tD11 %f D12 %f D22 %f D33 %f\nEta %f\n', D11_interp,D12_interp,D22_interp,D33_interp,etaTargetLocal);
