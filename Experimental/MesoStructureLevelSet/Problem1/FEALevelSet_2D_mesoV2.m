...
% ------------------------
% Version 2, 2D meso structure, optimization
% ----------------------

function [displacmentY,cost,maxDisplacement] = FEALevelSet_2D_mesoV2(xpoints,ypoints,zpoints, doplot)
...
if(problem ==1)
    ...
    % The control points go one element past the size of the unitcell
    unitCellSize = 4; % 3 by 3
    controlPointSpan = unitCellSize+2;
    cellAndControlPoints = 1:1:controlPointSpan;
    splineXX = cellAndControlPoints(2:controlPointSpan-1)/controlPointSpan;
    splineXX =splineXX -1/(controlPointSpan*2);%minus half the width, so we get the center of the element.
    splineYY = splineXX;
