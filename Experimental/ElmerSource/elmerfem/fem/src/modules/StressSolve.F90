...
!------------------------------------------------------------------------------
    ...
    INTEGER :: numNodesForE
    INTEGER :: modelDim

     TYPE(Matrix_t), POINTER   :: Atemp
     ...
     RelIntegOrder = ListGetInteger( SolverParams,'Relative Integration Order',Found)
        ...
        ! -----------------Anthony------------
        ! open file for writing local stiffness matrix
      open (unit = 101, file = "lstiffness.txt")
      open (unit = 102, file = "lmass.txt")
       Atemp => Solver % Matrix
...
!------------------------------------------------------------------------------
       ...
       ! Anthony Modify
        !TYPE(Variable_t), POINTER :: xxxx
        !xxxx => Solver % Variable
        !DOFs = xxxx % DOFs
        !INTEGER, POINTER :: Indexes(:)
        !Indexes => GetIndexStore()
        !n = GetElementDOFs( Indexes, Element, Solver )
        numNodesForE =GetElementNOFNodes(Element)
         modelDim = CurrentModel % Dimension
        !ALLOCATE( G(DOFs*n,DOFs*n), F(DOFs*n) )
        ! G and F in the allocate are the local stiffness and force matrixs

        write(101,*) '--',t ! new element, stiffness
        write(102,*) '--',t ! new element, mass

        DO i = 1, STDOFs*N
            DO j = 1, STDOFs*N
               write(101,*) STIFF(i,j) ! stiffness
